...
namespace WebGL
{
    ...
    public class WebGLRenderingContext : CanvasRenderingContext
    {
        ...
        internal WebGLTexture m_blackTexture2D;
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            {
                this.m_textureUnits[i].texture2DBinding = null;
        ...
        public void bindTexture(GLenum target, WebGLTexture texture)
        {
            ...
            {
                this.m_textureUnits[(int)this.m_activeTextureUnit].texture2DBinding = texture;
        ...
        public void deleteTexture(WebGLTexture texture)
        {
            ...
            for (var i = 0; i < this.m_textureUnits.Length; ++i)
            {
                if (texture == this.m_textureUnits[i].texture2DBinding)
                {
                    this.m_textureUnits[i].texture2DBinding = null;
                }
        ...
        {
            if (this.isContextLostOrPending() || !this.m_validation.validateFramebufferFuncParameters("framebufferTexture2D", target, attachment))
            {
                return;
            }
            if (level != 0)
            {
                this.synthesizeGLError(GraphicsContext3D.INVALID_VALUE, "framebufferTexture2D", "level not 0");
                return;
            }
            if (texture != null && !texture.validate(this.contextGroup(), this))
            {
                this.synthesizeGLError(GraphicsContext3D.INVALID_OPERATION, "framebufferTexture2D", "no texture or texture not from this context");
                return;
            }
            // Don't allow the default framebuffer to be mutated; all current
            // implementations use an FBO internally in place of the default
            ...
            if (this.m_framebufferBinding == null || this.m_framebufferBinding.obj() == 0)
            {
                this.synthesizeGLError(GraphicsContext3D.INVALID_OPERATION, "framebufferTexture2D", "no framebuffer bound");
                return;
            }
            ...
            {
                case GraphicsContext3D.DEPTH_STENCIL_ATTACHMENT:
                    this.m_context.framebufferTexture2D(target, GraphicsContext3D.DEPTH_ATTACHMENT, textarget, textureObject, level);
                    this.m_context.framebufferTexture2D(target, GraphicsContext3D.STENCIL_ATTACHMENT, textarget, textureObject, level);
                    break;
                case GraphicsContext3D.DEPTH_ATTACHMENT:
                    this.m_context.framebufferTexture2D(target, attachment, textarget, textureObject, level);
                    break;
        ...
        public dynamic getParameter(GLenum pname)
        {
            ...
            switch (pname)
            {
                ...
                case GraphicsContext3D.TEXTURE_BINDING_2D:
                    return this.m_textureUnits[this.m_activeTextureUnit].texture2DBinding;
        ...
        internal void checkTextureCompleteness(String functionName, bool prepareToDraw)
        {
            ...
            {
                if ((this.m_textureUnits[ii].texture2DBinding != null && this.m_textureUnits[ii].texture2DBinding.needToUseBlackTexture(extensions))
                    || (this.m_textureUnits[ii].textureCubeMapBinding != null && this.m_textureUnits[ii].textureCubeMapBinding.needToUseBlackTexture(extensions)))
                {
                    ...
                    if (prepareToDraw)
                    {
                        ...
                        tex2D = this.m_blackTexture2D;
                        texCubeMap = this.m_blackTextureCubeMap;
                    }
                    else
                    {
                        tex2D = this.m_textureUnits[ii].texture2DBinding;
        ...
        internal void createFallbackBlackTextures1x1()
        {
            ...
            this.m_blackTexture2D = this.createTexture();
            this.m_context.bindTexture(GraphicsContext3D.TEXTURE_2D, this.m_blackTexture2D.obj());
        ...
        internal void restoreCurrentTexture2D()
        {
            this.bindTexture(GraphicsContext3D.TEXTURE_2D, this.m_textureUnits[this.m_activeTextureUnit].texture2DBinding);
        }
        ...
        {
            public WebGLTexture texture2DBinding;
